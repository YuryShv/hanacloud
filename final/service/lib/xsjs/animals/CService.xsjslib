...
this.fillData = function(stmt, oValueObject){
    	stmt.setString(1, oValueObject.aid.toString());
		stmt.setString(2, oValueObject.aname.toString());
        stmt.setString(3, oValueObject.akind.toString());
        stmt.setTimestamp(4, (new Date()).toISOString());
        stmt.setTimestamp(5, (new Date()).toISOString());
        stmt.executeUpdate();
        stmt.close();
}
